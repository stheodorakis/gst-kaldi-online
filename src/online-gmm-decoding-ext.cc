...
SingleUtteranceGmmDecoderExt::SingleUtteranceGmmDecoderExt(
...
void SingleUtteranceGmmDecoderExt::AdvanceDecoding() {
  ...
  bool fmllr_flag=false;
  if (fmllr_flag){  // possibly estimate fMLLR.
...
void SingleUtteranceGmmDecoderExt::FinalizeDecoding() {
  decoder_.FinalizeDecoding();
}

// gets Gaussian posteriors for purposes of fMLLR estimation.
// We exclude the silence phones from the Gaussian posteriors.
bool SingleUtteranceGmmDecoderExt::GetGaussianPosteriors(bool end_of_utterance,
                                                      GaussPost *gpost) {
...
void SingleUtteranceGmmDecoderExt::EstimateFmllr(bool end_of_utterance) {
  if (decoder_.NumFramesDecoded() == 0) {
    KALDI_WARN << "You have decoded no data so cannot estimate fMLLR.";
  }
...
bool SingleUtteranceGmmDecoderExt::HaveTransform() const {
  return (feature_pipeline_->HaveFmllrTransform());
}

void SingleUtteranceGmmDecoderExt::GetAdaptationState(
...
bool SingleUtteranceGmmDecoderExt::RescoringIsNeeded() const {
...
bool SingleUtteranceGmmDecoderExt::EndpointDetected(
...
void SingleUtteranceGmmDecoderExt::GetLattice(bool rescore_if_needed,
...
void SingleUtteranceGmmDecoderExt::GetBestPath(bool end_of_utterance,
