...
/**
 * SECTION:element-kaldigmm2onlinedecoder
 *
 * Converts speech to text using Kaldi's SingleUtteranceGmm2Decoder.
 *
 * <title>Example launch line</title>
 * |[
 * GST_PLUGIN_PATH=. gst-launch-1.0 --gst-debug="kaldigmm2onlinedecoder:5" -q \
 * filesrc location=123_456.wav ! decodebin ! audioconvert ! audioresample ! \
 * kaldigmm2onlinedecoder model=gmm2_online_ivector_online/final.mdl fst=tri3b/graph/HCLG.fst word-syms=tri3b/graph/words.txt \
 * feature-type=mfcc mfcc-config=gmm2_online_ivector_online/conf/mfcc.conf \
...
#include "./gstkaldigmm2onlinedecoder.h"
...
GST_DEBUG_CATEGORY_STATIC(gst_kaldigmm2onlinedecoder_debug);
#define GST_CAT_DEFAULT gst_kaldigmm2onlinedecoder_debug
...
static guint gst_kaldigmm2onlinedecoder_signals[LAST_SIGNAL];

#define gst_kaldigmm2onlinedecoder_parent_class parent_class
G_DEFINE_TYPE(Gstkaldigmm2onlinedecoder, gst_kaldigmm2onlinedecoder,
              GST_TYPE_ELEMENT);

static void gst_kaldigmm2onlinedecoder_load_phone_syms(Gstkaldigmm2onlinedecoder * filter,
                                                        const GValue * value);

static void gst_kaldigmm2onlinedecoder_load_word_syms(Gstkaldigmm2onlinedecoder * filter,
                                                       const GValue * value);

static void gst_kaldigmm2onlinedecoder_load_model(Gstkaldigmm2onlinedecoder * filter,
                                                   const GValue * value);

static void gst_kaldigmm2onlinedecoder_load_fst(Gstkaldigmm2onlinedecoder * filter,
                                                 const GValue * value);

static void gst_kaldigmm2onlinedecoder_load_lm_fst(Gstkaldigmm2onlinedecoder * filter,
                                                    const GValue * value);

static void gst_kaldigmm2onlinedecoder_load_big_lm(Gstkaldigmm2onlinedecoder * filter,
                                                    const GValue * value);

static void gst_kaldigmm2onlinedecoder_load_word_boundary_info(Gstkaldigmm2onlinedecoder * filter,
                                                                const GValue * value);


static void gst_kaldigmm2onlinedecoder_set_property(GObject * object,
                                                     guint prop_id,
                                                     const GValue * value,
                                                     GParamSpec * pspec);

static void gst_kaldigmm2onlinedecoder_get_property(GObject * object,
                                                     guint prop_id,
                                                     GValue * value,
                                                     GParamSpec * pspec);

static gboolean gst_kaldigmm2onlinedecoder_sink_event(GstPad * pad,
                                                       GstObject * parent,
                                                       GstEvent * event);

static GstFlowReturn gst_kaldigmm2onlinedecoder_chain(GstPad * pad,
                                                       GstObject * parent,
                                                       GstBuffer * buf);

static GstStateChangeReturn gst_kaldigmm2onlinedecoder_change_state(
    GstElement *element, GstStateChange transition);

static gboolean gst_kaldigmm2onlinedecoder_query(GstPad *pad, GstObject * parent, GstQuery * query);

static void gst_kaldigmm2onlinedecoder_finalize(GObject * object);

/* GObject vmethod implementations */

/* initialize the kaldigmm2onlinedecoder's class */
static void gst_kaldigmm2onlinedecoder_class_init(
    Gstkaldigmm2onlinedecoderClass * klass) {
  ...
  gobject_class->set_property = gst_kaldigmm2onlinedecoder_set_property;
  gobject_class->get_property = gst_kaldigmm2onlinedecoder_get_property;
  gobject_class->finalize = gst_kaldigmm2onlinedecoder_finalize;

  gstelement_class->change_state = gst_kaldigmm2onlinedecoder_change_state;
  ...
  gst_kaldigmm2onlinedecoder_signals[PARTIAL_RESULT_SIGNAL] = g_signal_new(
      "partial-result", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(Gstkaldigmm2onlinedecoderClass, partial_result),
      NULL,
      NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
      G_TYPE_STRING);

  gst_kaldigmm2onlinedecoder_signals[FINAL_RESULT_SIGNAL] = g_signal_new(
      "final-result", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(Gstkaldigmm2onlinedecoderClass, final_result),
      NULL,
      NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
      G_TYPE_STRING);

  gst_kaldigmm2onlinedecoder_signals[FULL_FINAL_RESULT_SIGNAL] = g_signal_new(
      "full-final-result", G_TYPE_FROM_CLASS(klass), G_SIGNAL_RUN_LAST,
      G_STRUCT_OFFSET(Gstkaldigmm2onlinedecoderClass, full_final_result),
      NULL,
      NULL, kaldi_marshal_VOID__STRING, G_TYPE_NONE, 1,
      G_TYPE_STRING);

  gst_element_class_set_details_simple(
      gstelement_class, "KaldiGMM2OnlineDecoder", "Speech/Audio",
      "Convert speech to text", "Stavros Theodorakis <sth@atrion.gr>");
 ...
 */
static void gst_kaldigmm2onlinedecoder_init(
    Gstkaldigmm2onlinedecoder * filter) {
  bool tmp_bool;
  int32 tmp_int;
  uint32 tmp_uint;
  float tmp_float;
  ...
  gst_pad_set_event_function(
      filter->sinkpad,
      GST_DEBUG_FUNCPTR(gst_kaldigmm2onlinedecoder_sink_event));
  gst_pad_set_chain_function(
      filter->sinkpad, GST_DEBUG_FUNCPTR(gst_kaldigmm2onlinedecoder_chain));
  gst_pad_set_query_function(
      filter->sinkpad, GST_DEBUG_FUNCPTR(gst_kaldigmm2onlinedecoder_query));
  ...
  filter->feature_config_cmd = new OnlineFeaturePipelineCommandLineConfig();
  filter->feature_config_cmd->Register(filter->simple_options);

  filter->endpoint_config = new OnlineEndpointConfig();
  filter->gmm2_decoding_config = new OnlineGmmDecodingConfig();
  //filter->gmm2_decoding_threaded_config = new OnlineNnet2DecodingThreadedConfig();

  filter->cmvn_state = new OnlineCmvnState();

  filter->endpoint_config->Register(filter->simple_options);
  filter->gmm2_decoding_config->Register(filter->simple_options);


  // will be set later
  filter->feature_config = NULL;
...
static void gst_kaldigmm2onlinedecoder_set_property(GObject * object,
  ...
  Gstkaldigmm2onlinedecoder *filter = GST_KALDIGMM2ONLINEDECODER(object);
  ...
  switch (prop_id) {
    case PROP_SILENT:
      ...
      break;
    case PROP_MODEL:
      gst_kaldigmm2onlinedecoder_load_model(filter, value);
      break;
    case PROP_FST:
      gst_kaldigmm2onlinedecoder_load_fst(filter, value);
      break;
    case PROP_WORD_SYMS:
      gst_kaldigmm2onlinedecoder_load_word_syms(filter, value);
      break;
    case PROP_PHONE_SYMS:
      gst_kaldigmm2onlinedecoder_load_phone_syms(filter, value);
      break;
    ...
    case PROP_TRACEBACK_PERIOD_IN_SECS:
      ...
      break;
    case PROP_LM_FST:
      gst_kaldigmm2onlinedecoder_load_lm_fst(filter, value);
      break;
    case PROP_BIG_LM_CONST_ARPA:
      gst_kaldigmm2onlinedecoder_load_big_lm(filter, value);
      break;
    case PROP_WORD_BOUNDARY_FILE:
      gst_kaldigmm2onlinedecoder_load_word_boundary_info(filter, value);
      break;
    case PROP_USE_THREADED_DECODER:
      filter->use_threaded_decoder = g_value_get_boolean(value);
      if (filter->use_threaded_decoder) {
        GST_ERROR_OBJECT(filter, "Thread feature is not implemented for GMMs.");
        //filter->gmm2_decoding_threaded_config->Register(filter->simple_options);
      } else {
        filter->gmm2_decoding_config->Register(filter->simple_options);
      }
      break;
        ...
        if (G_VALUE_HOLDS_STRING(value)) {
          ...
          if (strlen(adaptation_state_string) > 0) {
            ...
            try {
              ...
              filter->adaptation_state = new OnlineGmmAdaptationState();
            }
          } else {
...
static std::vector<PhoneAlignmentInfo> gst_kaldigmm2onlinedecoder_phone_alignment(
    Gstkaldigmm2onlinedecoder * filter, const std::vector<int32>& alignment) {
...
static std::vector<WordAlignmentInfo>  gst_kaldigmm2onlinedecoder_word_alignment(
    Gstkaldigmm2onlinedecoder * filter, const Lattice &lat) {
...
static void gst_kaldigmm2onlinedecoder_scale_lattice(
        Gstkaldigmm2onlinedecoder * filter, CompactLattice &clat) {
...
static std::string gst_kaldigmm2onlinedecoder_words_to_string(
    Gstkaldigmm2onlinedecoder *filter, const std::vector<int32> &words) {
...
static std::string gst_kaldigmm2onlinedecoder_words_in_hyp_to_string(
    Gstkaldigmm2onlinedecoder *filter, const std::vector<WordInHypothesis> &words) {
  std::vector<int32> word_ids;
  for (size_t i = 0; i < words.size(); i++) {
    word_ids.push_back(words[i].word_id);
  }
  return gst_kaldigmm2onlinedecoder_words_to_string(filter, word_ids);
}

static std::vector<NBestResult> gst_kaldigmm2onlinedecoder_nbest_results(
    Gstkaldigmm2onlinedecoder * filter, CompactLattice &clat) {

  std::vector<NBestResult> nbest_results;

  // FIXME: is it needed?
  //gst_kaldigmm2onlinedecoder_scale_lattice(filter, clat);
  Lattice lat;
    ...
    if (i == 0) {
      if (filter->do_phone_alignment) {
        nbest_result.phone_alignment =
            gst_kaldigmm2onlinedecoder_phone_alignment(filter, alignment);
      }
      if (filter->word_boundary_info) {
        nbest_result.word_alignment = gst_kaldigmm2onlinedecoder_word_alignment(filter, nbest_lats[i]);
      }
    }
...
static std::string gst_kaldigmm2onlinedecoder_full_final_result_to_json(
    Gstkaldigmm2onlinedecoder * filter,
  ...
  if (full_final_result.nbest_results.size() > 0) {
    BaseFloat frame_shift = filter->feature_config->FrameShiftInSeconds();
    ...
    for(std::vector<NBestResult>::const_iterator it = full_final_result.nbest_results.begin();
      ...
      json_object_set_new(nbest_result_json_object, "transcript",
                          json_string(gst_kaldigmm2onlinedecoder_words_in_hyp_to_string(filter, nbest_result.words).c_str()));
...
static void gst_kaldigmm2onlinedecoder_final_result(
    Gstkaldigmm2onlinedecoder * filter, CompactLattice &clat,
    guint *num_words) {
  if (clat.NumStates() == 0) {
  ...
  full_final_result.nbest_results = gst_kaldigmm2onlinedecoder_nbest_results(filter, clat);

  if (full_final_result.nbest_results.size() > 0) {
    std::string best_transcript = gst_kaldigmm2onlinedecoder_words_in_hyp_to_string(filter, full_final_result.nbest_results[0].words);
    ...
    *num_words = full_final_result.nbest_results[0].words.size();
    ...
    if (hyp_length > 0) {
      ...
      g_signal_emit(filter, gst_kaldigmm2onlinedecoder_signals[FINAL_RESULT_SIGNAL], 0, best_transcript.c_str());

      std::string full_final_result_as_json =
          gst_kaldigmm2onlinedecoder_full_final_result_to_json(filter, full_final_result);
      GST_DEBUG_OBJECT(filter, "Final JSON: %s", full_final_result_as_json.c_str());
      g_signal_emit(filter, gst_kaldigmm2onlinedecoder_signals[FULL_FINAL_RESULT_SIGNAL], 0, full_final_result_as_json.c_str());

    }
  }
}

static void gst_kaldigmm2onlinedecoder_partial_result(
    Gstkaldigmm2onlinedecoder * filter, const Lattice lat) {
  std::vector<int32> words;
  std::vector<int32> alignment;
  LatticeWeight weight;
  GetLinearSymbolSequence(lat, &alignment, &words, &weight);
  std::string transcript = gst_kaldigmm2onlinedecoder_words_to_string(filter, words);
  GST_DEBUG_OBJECT(filter, "Partial: %s", transcript.c_str());
  if (transcript.length() > 0) {
    /* Emit a signal for applications. */
    g_signal_emit(filter,
                  gst_kaldigmm2onlinedecoder_signals[PARTIAL_RESULT_SIGNAL], 0,
                  transcript.c_str());
  }
}

static bool gst_kaldigmm2onlinedecoder_rescore_big_lm(
    Gstkaldigmm2onlinedecoder * filter, CompactLattice &clat, CompactLattice &result_lat) {
...
// static void gst_kaldigmm2onlinedecoder_threaded_decode_segment(Gstkaldigmm2onlinedecoder * filter,
//                                                       bool &more_data,
//                                                       int32 chunk_length,
//                                                       BaseFloat traceback_period_secs,
//                                                       Vector<BaseFloat> *remaining_wave_part) {
//     SingleUtteranceNnet2DecoderThreaded decoder(*(filter->gmm2_decoding_threaded_config),
...
//         if (gst_kaldigmm2onlinedecoder_rescore_big_lm(filter, clat, rescored_lat)) {
//           clat = rescored_lat;
//         }
//       }
...
static void gst_kaldigmm2onlinedecoder_unthreaded_decode_segment(Gstkaldigmm2onlinedecoder * filter,
                                                        bool &more_data,
                                                        int32 chunk_length,
                                                        BaseFloat traceback_period_secs) {

  filter->gmm2_decoding_config->model_rxfilename = filter->model_rspecifier;
  OnlineGmmDecodingModels gmm_models(*(filter->gmm2_decoding_config));
  OnlineFeaturePipeline feature_pipeline(*(filter->feature_config));
  
  //feature_pipeline.SetAdaptationState(*(filter->adaptation_state));

  SingleUtteranceGmmDecoderExt decoder(*(filter->gmm2_decoding_config),
                                      gmm_models,
                                      feature_pipeline,
                                      *(filter->decode_fst),
                                      *(filter->adaptation_state));
  ...
  if (SET_CMVN_STATE){
    decoder.FeaturePipeline().SetCmvnState(*(filter->cmvn_state));
  }
  ...
  while (true) {
    ...
    decoder.FeaturePipeline().AcceptWaveform(filter->sample_rate, wave_part);
    if (!more_data) {
    ...
    if (!more_data) {
      SET_CMVN_STATE = false;
      break;
    }
    if (filter->do_endpointing && decoder.EndpointDetected(*(filter->endpoint_config))){
      GST_DEBUG_OBJECT(filter, "Endpoint detected!");
      //Get cvmvn state
      GST_DEBUG_OBJECT(filter, "Getting cvmvn state");
      decoder.FeaturePipeline().GetCmvnState(filter->cmvn_state);
      GST_DEBUG_OBJECT(filter, "Got cvmvn state");
      SET_CMVN_STATE = true;
      break;
    }
...
static void gst_kaldigmm2onlinedecoder_loop(
    Gstkaldigmm2onlinedecoder * filter) {
  ...
  while (more_data) {
    if (filter->use_threaded_decoder) {
      GST_ERROR_OBJECT(filter, "Thread feature is not implemented yet for GMMs.");
      //gst_kaldigmm2onlinedecoder_threaded_decode_segment(filter, more_data, chunk_length, traceback_period_secs, &remaining_wave_part);
    } else {
      gst_kaldigmm2onlinedecoder_unthreaded_decode_segment(filter, more_data, chunk_length, traceback_period_secs);
    }
...
static gboolean
gst_kaldigmm2onlinedecoder_query (GstPad *pad, GstObject * parent, GstQuery * query) {
  gboolean ret;
  Gstkaldigmm2onlinedecoder *filter;

  filter = GST_KALDIGMM2ONLINEDECODER(parent);

  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_CAPS: {
      if (filter->feature_config == NULL) {
        filter->feature_config = new OnlineFeaturePipelineConfig(*(filter->feature_config_cmd));
        filter->sample_rate = (int) filter->feature_config->mfcc_opts.frame_opts.samp_freq;
      }
  ...
  switch (GST_EVENT_TYPE(event)) {
    case GST_EVENT_SEGMENT: {
      ...
      gst_pad_start_task(filter->srcpad,
                         (GstTaskFunction) gst_kaldigmm2onlinedecoder_loop,
                         filter, NULL);
...
static GstFlowReturn gst_kaldigmm2onlinedecoder_chain(GstPad * pad,
                                                       GstObject * parent,
                                                       GstBuffer * buf) {
  Gstkaldigmm2onlinedecoder *filter = GST_KALDIGMM2ONLINEDECODER(parent);
...
static void
gst_kaldigmm2onlinedecoder_load_word_syms(Gstkaldigmm2onlinedecoder * filter,
                                           const GValue * value) {
    if (G_VALUE_HOLDS_STRING(value)) {
...
static void
gst_kaldigmm2onlinedecoder_load_phone_syms(Gstkaldigmm2onlinedecoder * filter,
                                            const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldigmm2onlinedecoder_load_word_boundary_info(Gstkaldigmm2onlinedecoder * filter,
                                                    const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldigmm2onlinedecoder_load_model(Gstkaldigmm2onlinedecoder * filter,
                                       const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
    // Check if the model filename is not empty
...
static void
gst_kaldigmm2onlinedecoder_load_fst(Gstkaldigmm2onlinedecoder * filter,
                                     const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldigmm2onlinedecoder_load_lm_fst(Gstkaldigmm2onlinedecoder * filter,
                                        const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static void
gst_kaldigmm2onlinedecoder_load_big_lm(Gstkaldigmm2onlinedecoder * filter,
                                        const GValue * value) {
  if (G_VALUE_HOLDS_STRING(value)) {
    gchar* str = g_value_dup_string(value);
...
static bool
gst_kaldigmm2onlinedecoder_allocate(
    Gstkaldigmm2onlinedecoder * filter) {
...
static bool gst_kaldigmm2onlinedecoder_deallocate(
    Gstkaldigmm2onlinedecoder * filter) {
...
static GstStateChangeReturn gst_kaldigmm2onlinedecoder_change_state(
  ...
  Gstkaldigmm2onlinedecoder *filter = GST_KALDIGMM2ONLINEDECODER(element);

  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      if (!gst_kaldigmm2onlinedecoder_allocate(filter))
  ...
  switch (transition) {
    case GST_STATE_CHANGE_READY_TO_NULL:
      gst_kaldigmm2onlinedecoder_deallocate(filter);
      break;
    default:
      break;
  }
 ...
 */
static gboolean kaldigmm2onlinedecoder_init(
    GstPlugin * kaldigmm2onlinedecoder) {
  /* debug category for fltering log messages
   *
   * exchange the string 'Template kaldigmm2onlinedecoder' with your description
   */
  GST_DEBUG_CATEGORY_INIT(gst_kaldigmm2onlinedecoder_debug,
                          "kaldigmm2onlinedecoder", 0,
                          "Template kaldigmm2onlinedecoder");

  return gst_element_register(kaldigmm2onlinedecoder,
                              "kaldigmm2onlinedecoder", GST_RANK_NONE,
                              GST_TYPE_KALDIGMM2ONLINEDECODER);
}
...
/* gstreamer looks for this structure to register kaldigmm2onlinedecoders
 *
 * exchange the string 'Template kaldigmm2onlinedecoder' with your kaldigmm2onlinedecoder description
 *
 * License is specified as "unknown" because gstreamer doesn't recognize "Apache" as
 * a license and blacklists the module :S
 */
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, kaldigmm2onlinedecoder,
                  "kaldigmm2onlinedecoder",
                  kaldigmm2onlinedecoder_init, VERSION, "unknown", "GStreamer",
                  "http://gstreamer.net/")
